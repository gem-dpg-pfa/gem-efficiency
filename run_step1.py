...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
            description='''Scripts that: \n\t-Execute step1_hit_perLumi_analysis.py\n\t-Submit to HTCondor\n''',
            epilog="""Typical execution\n\t python run_step1 -r 342154 --submit True """,
...
+JobFlavour = "workday"
            '''.format(path=args.outputpath, submit_time=str(time.time())))
        ...
        for file_index in range(n_chunk+1):
            ...
            for idx, l in enumerate(fileList):
                if idx < args.num_input*(file_index+1) and idx >= args.num_input*file_index:
                    ...
                    l = l+","
                    sublist = sublist+l
            command = "python step1_hit_perLumi_analysis.py --input_file="+sublist
            command += " --output "+args.outputpath+"/step1_"+run+"_"+str(file_index)+".root --nevents "+str(args.execute)
...
arguments = {arg}
queue
                    '''.format(arg = "--input_file="+sublist+" --output "+args.outputpath+"/step1_"+run+"_"+str(file_index)+".root --nevents -1" ))
